...
class CarDispatcherROS(threading.Thread):
    def __init__(self,pipe):
        ...
        self.loading_point.pose.position.x=rospy.get_param('loading_x',default=2.26)
        self.loading_point.pose.position.y=rospy.get_param('loading_y',default=-3.28)
        ...
        #viapoint-1(0.10,-7.86,-180)
        
        self.viapoint_1=PoseStamped()
        self.viapoint_1.header.frame_id='map'
        self.viapoint_1.pose.position.x=rospy.get_param('vp1_x',default=0.10)
        self.viapoint_1.pose.position.y=rospy.get_param('vp1_y',default=-7.86)
        ...
        #unloading point    (-2.31,-6.21,90)
        self.unloading_point=PoseStamped()
        self.unloading_point.header.frame_id='map'
        self.unloading_point.pose.position.x=rospy.get_param('unloading_x',default=-2.29)
        self.unloading_point.pose.position.y=rospy.get_param('unloading_y',default=-5.95)
        ...
        #viapoint-2 (0.38,-5.84,45)
        self.viapoint_2=PoseStamped()   
        self.viapoint_2.header.frame_id='map'   
        self.viapoint_2.pose.position.x=rospy.get_param('vp2_x',default=1.01)
        self.viapoint_2.pose.position.y=rospy.get_param('vp2_y',default=-4.0)
        ...
        self.rect_ymin=-5.5
        self.model_prev=0.6
        self.rect_model_vel=0.45
        self.pth=Path()
        self.pth.header.frame_id='map'
        self.pth_length=self.rect_ymax-self.rect_ymin-self.model_prev
        self.rect_time=self.pth_length/self.rect_model_vel
        
        pstart=PoseStamped()
        pstart.pose.position.x=0.94
        ...
        step_y=(pend.pose.position.y-pstart.pose.position.y-self.model_prev)/num_pts
        
        p=PoseStamped()
        for i in range(num_pts):
            p.pose.position.y=(i+1)*step_y+pstart.pose.position.y+self.model_prev
        ...
        self.ref_model.pose.position.z=5.0
        ...
        self.goal_reached=rospy.Subscriber('/move_base/TebLocalPlannerROS/GOAL_OR_NOT',Int16,self.goal_reach_callback)
        ...
        self.pub_carline=rospy.Publisher('dispatcher/line_start', UInt8,queue_size=1)
        self.pub_calib=rospy.Publisher('dispatcher/amcl_calib',UInt8,queue_size=1)
        
        self.timer_model = rospy.Timer(rospy.Duration(0.1),self.model_callback)
        self.clearing_request()
        rospy.loginfo('clearing')
    def goal_reach_callback(self,goal_reached_data):
        #rospy.loginfo('goal_reached_data=%d',goal_reached_data.data)
        if(self.nav_status==3 and goal_reached_data.data==1):
            self.vp2_reach_status=True
            self.nav_status=4    
            self.target.pose=self.waiting_point.pose
            self.pub_target.publish(self.target)  
            rospy.loginfo("reach p2")    
            msg_line_start=UInt8()
            msg_line_start.data=1
            self.pub_carline.publish(msg_line_start)
            #self.pipe.send(('message','vp2-reached'))
        if(self.nav_status==0 and goal_reached_data.data==1):
            self.clearing_request()
            rospy.loginfo('clearing') 
        if(self.nav_status==2 and goal_reached_data.data==1):
            self.clearing_request()
            rospy.loginfo('clearing')
        if(self.nav_status==4 and goal_reached_data.data==1):
            self.clearing_request()
            rospy.loginfo('clearing')            
    ...
    def amcl_callback(self,amcl_data):
       # rospy.loginfo('amcl')
        ...
        if self.nav_status==1 and abs(self.x-self.viapoint_1.pose.position.x)<1.0 and abs(self.y-self.viapoint_1.pose.position.y)<1.0:
            self.nav_status=2
            self.target.pose=self.unloading_point.pose
            self.pub_target.publish(self.target)
            rospy.loginfo('AMCL REACH P1')
    ...
    def model_callback(self,event):
        ...
        if(self.s_path_approaching==True):
            ...
            if(t<self.rect_time):
                ...
                self.ref_model.pose.position.x=0.94
                #self.ref_model.pose.position.y=self.ref_model.prev+self.rect_ymin+t*self.rect_model_vel
...
if __name__ == "__main__":
        ...
        while True:
            ...
            #    mmsg=UInt8()
            #    mmsg.data=1
            #    car_dispatcher.pub_calib.publish(mmsg)
            #    rospy.loginfo('amcl calib')
            # elif key=='P' or key=='p':
